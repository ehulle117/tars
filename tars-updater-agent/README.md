...
- **Weekly Digest**: Summarizes pending baremetal OS updates and container updates, posted to a Discord channel via the Tars bot.
...
- **Persistent Findings History**: Every scanned vulnerability and pending update is also written to `vulnerability_findings` / `update_findings` tables in the same SQLite DB (with first/last seen timestamps), independent of Discord post dedup — so findings remain queryable even after the alert has been posted. View them with:
  ```bash
  python -m app.report
  ```
...
1. Rename `agent-config.example.yaml` to `agent-config.yaml`, and set the `DISCORD_BOT_TOKEN` / `DIGEST_CHANNEL_ID` env vars (see `docker-compose.example.yml`).
