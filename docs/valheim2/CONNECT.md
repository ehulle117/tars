...
did the Tailscale steps for the old server, you can skip straight to
**Step 3**.
...
*(Skip if you already have one from the old server.)*
...
*(Skip if you already have it installed and running.)*
...
Open the share link you were given for **this server** while signed in to
Tailscale, and click **Accept**. This is a separate share from the old
server's link, even if the same person runs both.
...
4. Type in the server address you were given (see the checklist at the top),
   ...
   Note the port — **2458**, not 2456. That's how this server is told apart
   from the old one.
...
**Is this the same server as before?**
No — this is the new 1.0 world, running as its own separate server. The old
world is still up at its own address until it's retired.
